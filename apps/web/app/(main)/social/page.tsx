...
function FriendsTab() {
  ...
  const load = async () => { try { setFriends(await apiFetch('/social/friends')) } catch {} }
  useEffect(() => { load() }, [])

  const add = async () => {
    if (!input.trim()) return
    setLoading(true)
    try { await apiFetch('/social/friends', { method: 'POST', body: JSON.stringify({ friendId: input.trim() }) }); setInput(''); load() }
    finally { setLoading(false) }
  }

  const remove = async (friendId: string) => {
    await apiFetch(`/social/friends/${friendId}`, { method: 'DELETE' })
    load()
  }
...
function FavoritesTab() {
  ...
  const load = async () => { try { setFavorites(await apiFetch('/social/favorites')) } catch {} }
  useEffect(() => { load() }, [])

  const add = async () => {
    if (!input.trim()) return
    setLoading(true)
    try { await apiFetch('/social/favorites', { method: 'POST', body: JSON.stringify({ targetId: input.trim(), targetType: type }) }); setInput(''); load() }
    finally { setLoading(false) }
  }

  const remove = async (targetId: string) => {
    await apiFetch(`/social/favorites/${targetId}`, { method: 'DELETE' })
    load()
  }

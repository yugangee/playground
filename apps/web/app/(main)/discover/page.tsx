...
import type { Team, League } from '@playground/shared'
...
function TeamsDiscover() {
  ...
  const search = async () => {
    ...
    try {
      ...
      setTeams(await apiFetch(`/discover/teams?${params}`))
    } finally { setLoading(false) }
  }
  useEffect(() => { search() }, [])

  const favorite = async (teamId: string) => {
    await apiFetch('/social/favorites', { method: 'POST', body: JSON.stringify({ targetId: teamId, targetType: 'team' }) })
  }
...
function LeaguesDiscover() {
  ...
  const search = async () => {
    ...
    try {
      ...
      setLeagues(await apiFetch(`/discover/leagues?${params}`))
    } finally { setLoading(false) }
  }
  useEffect(() => { search() }, [])

  const favorite = async (leagueId: string) => {
    await apiFetch('/social/favorites', { method: 'POST', body: JSON.stringify({ targetId: leagueId, targetType: 'league' }) })
  }

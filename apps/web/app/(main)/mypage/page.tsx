...
export default function MypagePage() {
  ...
  useEffect(() => {
    fetchUserAttributes().then(attrs => setName(attrs.name ?? '')).catch(() => {})
    apiFetch('/team').then(setTeams).catch(() => {})
  }, [])
  ...
  const initials = name ? name.slice(0, 2).toUpperCase() : (user?.username?.charAt(0).toUpperCase() ?? '?')

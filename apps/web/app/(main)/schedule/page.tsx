...
import type { Match, Announcement, Poll } from '@playground/shared'
...
export default function SchedulePage() {
  ...
  const loadMatches = async () => {
    if (!teamId) return
    try { setMatches(await apiFetch(`/schedule/matches?teamId=${teamId}`)) } catch {}
  }
...
function MatchesTab({ teamId, isLeader, matches, onRefresh }: { teamId: string; isLeader: boolean; matches: Match[]; onRefresh: () => void }) {
  ...
  const updateStatus = async (id: string, status: string) => {
    await apiFetch(`/schedule/matches/${id}`, { method: 'PATCH', body: JSON.stringify({ status }) })
    onRefresh()
  }
...
function MatchForm({ teamId, onSuccess }: { teamId: string; onSuccess: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    try { await apiFetch('/schedule/matches', { method: 'POST', body: JSON.stringify(form) }); onSuccess() }
...
function AttendanceRow({ matchId }: { matchId: string }) {
  ...
  const respond = async (s: string) => {
    await apiFetch(`/schedule/matches/${matchId}/attendance`, { method: 'PUT', body: JSON.stringify({ status: s }) })
    setStatus(s)
  }
...
function AnnouncementsTab({ teamId, isLeader }: { teamId: string; isLeader: boolean }) {
  ...
  const load = async () => {
    try { setItems(await apiFetch(`/schedule/announcements?teamId=${teamId}`)) } catch {}
  }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await apiFetch('/schedule/announcements', { method: 'POST', body: JSON.stringify({ ...form, teamId }) })
...
function PollsTab({ teamId, isLeader }: { teamId: string; isLeader: boolean }) {
  ...
  const load = async () => {
    try { setItems(await apiFetch(`/schedule/polls?teamId=${teamId}`)) } catch {}
  }
  useEffect(() => { load() }, [teamId])

  const loadVotes = async (pollId: string) => {
    const data = await apiFetch(`/schedule/polls/${pollId}/votes`)
  ...
  const vote = async (pollId: string, optionIndex: number) => {
    await apiFetch(`/schedule/polls/${pollId}/vote`, { method: 'POST', body: JSON.stringify({ optionIndex }) })
    loadVotes(pollId)
  }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await apiFetch('/schedule/polls', { method: 'POST', body: JSON.stringify({ ...form, teamId }) })

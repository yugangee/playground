...
import type { League } from '@playground/shared'
...
export default function LeaguePage() {
  ...
  const load = async () => {
    ...
    try { setLeagues(await apiFetch(`/league?organizerTeamId=${teamId}`)) }
...
function CreateForm({ teamId, onSuccess, onCancel }: { teamId: string; onSuccess: () => void; onCancel: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    try { await apiFetch('/league', { method: 'POST', body: JSON.stringify(form) }); onSuccess() }
...
function LeagueDetail({ league, onBack, isLeader }: { league: League; onBack: () => void; isLeader: boolean }) {
  ...
  const loadTeams = async () => { try { setTeams(await apiFetch(`/league/${league.id}/teams`)) } catch {} }
  const loadMatches = async () => { try { setMatches(await apiFetch(`/league/${league.id}/matches`)) } catch {} }

  useEffect(() => { loadTeams(); loadMatches() }, [league.id])

  const invite = async () => {
    if (!inviteTeamId.trim()) return
    await apiFetch(`/league/${league.id}/teams`, { method: 'POST', body: JSON.stringify({ teamId: inviteTeamId }) })
    setInviteTeamId('')
    loadTeams()
  }
...
function MatchesSection({ leagueId, matches, onRefresh, isLeader }: { leagueId: string; matches: LeagueMatch[]; onRefresh: () => void; isLeader: boolean }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await apiFetch(`/league/${leagueId}/matches`, { method: 'POST', body: JSON.stringify(form) })
      setShowForm(false)
      onRefresh()
    } finally { setLoading(false) }
  }
  ...
  const saveResult = async (matchId: string) => {
    ...
    if (!s) return
    await apiFetch(`/league/${leagueId}/matches/${matchId}`, {
      method: 'PATCH',

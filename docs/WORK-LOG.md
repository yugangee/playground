...
총 23개 버그 (Critical 7, High 16) — Phase 1-5 원본 19개 + Phase 6 리그 심층 점검 4개.
...
## 리그 심층 점검 — 추가 버그 (Phase 6)

| ID | 심각도 | 페이지 | 설명 | 상태 |
|----|--------|--------|------|------|
| L1 | Critical | 리그 관리 | `MatchesSection`에서 `tn` 함수 미전달 → 경기 탭 런타임 크래시 | ✅ 완료 |
| L2 | High | 리그 관리 | 팀 초대가 UUID 수동 입력 → 사실상 사용 불가 | ✅ 완료 |
| L3 | High | 리그 관리 | 경기 수동 추가에서 홈/원정팀 UUID 입력 → 사용 불가 | ✅ 완료 |
| L4 | High | 리그 탐색 | "참가 신청하기"가 `/manage/league`로 이동만 → 신청 안됨 | ✅ 완료 |
...
- **Phase 6 완료** (L1, L2, L3, L4): 리그 심층 점검 — MatchesSection에 tn 함수 prop 전달, 팀 초대/경기 추가 UUID 입력을 select 드롭다운으로 교체, 리그 탐색 참가 신청 버튼 실제 API 호출로 변경

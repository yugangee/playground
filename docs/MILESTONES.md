...
#### DEMO-1. 토너먼트 데이터 레이어
- [ ] `types/tournament.ts` — TournamentTeam, TournamentMatch, TournamentRound, PlayerRoster 타입 정의
- [ ] `data/kja-tournament-51.ts` — 51회 대회 전체 데이터 (50개+ 팀, 55개 매치 슬롯, 서경 어벤져스 선수단 20명)
- [ ] `data/kja-tournament-50.ts` — 50회 결과 (YTN 우승, 동아일보 준우승, 뉴스1 3위, 서울경제 4위)

#### DEMO-2. 대진표 브래킷 시각화
- [ ] `TournamentBracket.tsx` — 좌측(54조)·우측(55조) 블록이 결승으로 수렴하는 레이아웃
- [ ] `MatchCard.tsx` — 팀명·스코어·상태별 스타일 (예정/진행중/완료/PK)
- [ ] 서경 어벤져스 경기 경로 하이라이트 (38조부터)
- [ ] 가로 스크롤 지원 (모바일 대응)
- [ ] `TeamRoster.tsx` — 등번호·이름·부서·역할 테이블

#### DEMO-3. `/league/kja-51` 대회 전용 페이지
- [ ] 히어로 섹션 (대회명, 참가 팀 수, 대회 방식)
- [ ] 탭 구조: 대진표 / 경기 일정 / 서경 어벤져스 / 이전 대회 / 대회 규정
- [ ] 서경 어벤져스 탭: 선수단 20명 + 부서별 분포 차트
- [ ] 대회 규정 탭: 경고 누적·몰수패·PK 규칙 아코디언 UI

#### DEMO-4. 네비게이션 & 랜딩 연동
- [ ] 사이드바에 "🏆 기자협회 축구대회" 바로가기 추가
- [ ] 랜딩 페이지 히어로/배너에 제51회 대회 소개 + CTA → `/league/kja-51`

#### DEMO-5. 스코어 입력 (데모용 로컬 상태)
- [ ] 총무용 스코어 입력 모달 (홈/어웨이 스코어, 승부차기 여부)
- [ ] 입력 결과가 브래킷에 자동 반영 (다음 라운드 승자 자동 배치)
- [ ] PK 승부차기 기록 지원
...
- [ ] **참석 현황 집계 표시 (참가 N명 / 불참 N명)** — KJA 7명 최소 인원 경고와 직결
- [ ] **7명 미달 경고 배지** — 경기 당일 출석이 7명 미만이면 빨간 경고 표시 (몰수패 방지)
- [ ] 최소 인원 미달 경고
- [ ] .ics 캘린더 내보내기 (구글/아이폰)
...
- [ ] 원클릭 초대 링크 생성 (초대코드 → URL 공유)
- [ ] 30초 온보딩 플로우 (링크 클릭 → 가입 → 팀 합류)
- [ ] 구장 즐겨찾기 (일정 등록 시 구장 정보 자동 연동, 내비 링크)
...
- [ ] QR 코드 생성 (경기별 고유 QR, 총무 화면에서 공유)
- [ ] QR 스캔 or 버튼 체크인 (도착 순서 기반 자동 번호 발급)
- [ ] **실시간 출석 카운터** — "현재 X/7명 확인" 표시, 7명 도달 시 그린 배지
- [ ] 용병(Guest) 임시 등록 — KJA에선 사전 등록 필수지만 일반 팀 경기용
- [ ] 출석 현황 실시간 업데이트

#### 2-B. 쿼터별 포메이션/라인업

- [ ] 쿼터 탭 (1Q~4Q 전환)
- [ ] 2D 축구장 + 드래그앤드롭 포지션 배치
- [ ] 스타팅/교체/대기 멤버 분류
- [ ] **PK 순서 관리** — 후반 종료 시점 출전 선수 자동 필터링 (KJA 규칙 반영)
- [ ] 팀원 앱에 라인업 실시간 동기화

#### 2-C. 경기 중 실시간 스코어보드

- [ ] `+ Goal` 버튼 → 득점자 선택 모달 → 어시스트 선택 모달
- [ ] 실시간 스코어 표시 (양팀 점수)
- [ ] 경기 이벤트 타임라인 (득점/교체/경고/퇴장)
- [ ] **경고 누적 트래커** — 선수별 경고 수 표시, 2장 시 다음 경기 출전정지 경고
- [ ] **경고 초기화 로직** — 8강까지 경고 1장 + 4강 진출 시 자동 초기화 (KJA 규칙)
- [ ] 백엔드 `PUT /matches/{id}/score`, `PUT /matches/{id}/goals` API 활용

#### 2-D. 경기 후 POTM 투표

- [ ] 경기 완료 처리 시 팀원 전원에게 POTM 투표 팝업 노출
- [ ] 실시간 투표 집계 + 1위에게 POTM 뱃지 자동 부여
- [ ] 투표 결과 경기 상세 페이지에 영구 저장
- [ ] 백엔드 `potmVotesReceived` 필드 활용 (구조 준비됨)
...
- [ ] 팀 내 리더보드 (득점왕, 어시스트왕, 출석왕, POTM 횟수)
- [ ] 시즌 리셋 / 감쇠 로직 구현

#### 3-B. 팀 통계 대시보드

- [ ] 팀 전체 성적 (승률, 평균 득점, 최장 연승)
- [ ] 팀 등급 승급 진행 바 (Rookie → Club → Crew → Elite → Legend)
- [ ] 상대 팀별 상대전적
- [ ] 홈/원정 성적 분리
...
- [ ] 경기별 사진/영상 업로드 (S3 uploads/ 활용)
- [ ] 경기별 미디어 자동 분류 및 갤러리 뷰
- [ ] AI 하이라이트 클립 자동 생성 (영상 분석 연계)
- [ ] **KT스카이라이프 AI 중계 연동 검토** — 제51회 KJA 대회에서 '포착' 솔루션 도입, 향후 연계 가능성

#### 3-D. 다종목 등급 커트라인 확정

- [ ] 동아리형 (러닝크루, 스노보드, 배드민턴) 등급표 설계
- [ ] 대전형 비축구 (농구, 야구, 배구, 아이스하키) 커트라인 설계
- [ ] 종목별 PIS 가중치 조정
...
- [ ] 경기 중 GPS 트래킹 시작/종료 (백그라운드 위치 추적)
- [ ] 2D 축구장 GPS 히트맵 렌더링 (활동 범위 컬러 시각화)
- [ ] 속도 구간별 분석 (걷기/조깅/달리기/스프린트 파이 차트)
- [ ] 수치 데이터 (총 이동 거리, 최고 속도, 스프린트 횟수)
- [ ] 2D 필드 리플레이어 (경기 중 움직임 궤적 애니메이션)
- [ ] `PlayerPerformance` DynamoDB 엔티티 설계 및 저장
- [ ] 스마트워치 연동 검토 (Phase 3)

...
export default function MyPage() {
  ...
  useEffect(() => {
    if (!user?.teamId) { setClub(null); return; }
    fetch(`${API}/clubs`)
      .then(r => r.json())
      .then(d => {
        const found = (d.clubs || []).find((c: any) => c.clubId === user.teamId);
        setClub(found || null);
      })
      .catch(() => {});
  }, [user]);
    ...
    try {
      ...
      const r = await fetch(`${API}/auth/profile`, {
        ...
        body: JSON.stringify({
          ...
          activeAreas: profileDraft.activeAreas.filter(a => a.sido),
          sports: profileDraft.sports,
        }),
      });
      if (!r.ok) {
  ...
  async function leaveTeam() {
    ...
    try {
      ...
      await fetch(`${API}/auth/profile`, {
        method: "PUT",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ hasTeam: false, teamSport: "", teamId: null, position: "" }),
      });
      await refresh();
      setEditing(false);
    } catch {
    ...
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button onClick={openTeamEdit} className="text-gray-500 hover:text-white transition-colors">
            <Pencil size={14} />
          </button>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: "팀명", value: club?.name || "-" },
            { label: "종목", value: user.teamSport || "-" },
            { label: "포지션", value: user.position || "-" },
            { label: "입단", value: (user as any).createdAt ? (user as any).createdAt.slice(0, 10) : "-" },
          ].map(({ label, value }) => (
            <div key={label} className="bg-white/5 rounded-lg px-4 py-3">
      ...
      {profileEditing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "var(--modal-overlay)" }} onClick={() => setProfileEditing(false)}>
          <div className="bg-[#111] border border-white/10 rounded-xl p-6 w-full max-w-sm space-y-4 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            {/* 활동 가능 지역 */}
            <div className="space-y-1">
              <label className="text-xs text-gray-400">활동 가능 지역 (최대 3개)</label>
              <div className="space-y-2">
                {profileDraft.activeAreas.map((area, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    <span className="text-xs text-gray-600 w-4 shrink-0">{i + 1}</span>
                    <Dropdown value={area.sido} placeholder="시/도" items={Object.keys(regionData)} onChange={(v) => {
                      const next = [...profileDraft.activeAreas];
                      next[i] = { sido: v, sigungu: "" };
                      setProfileDraft(p => ({ ...p, activeAreas: next }));
                    }} />
            ...
            {/* 관심 스포츠 */}
            <div className="space-y-1">
              <label className="text-xs text-gray-400">관심 스포츠</label>
              <div className="flex flex-wrap gap-2">
                {allSports.map(s => (
                  <button key={s} type="button"

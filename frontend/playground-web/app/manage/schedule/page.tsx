            ...
            <div key={m.id} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2">
                  <MatchStatusBadge status={m.status} />
                  {isLeader && (
                    <button onClick={async () => { if (confirm('이 경기를 삭제하시겠습니까?')) { await manageFetch(`/schedule/matches/${m.id}`, { method: 'DELETE' }); onRefresh() } }}
                      className="text-slate-300 hover:text-red-500 transition-colors" title="경기 삭제">
            ...
            <div key={a.id} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="rounded-lg bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-600">공지</span>
                  {isLeader && (
                    <button onClick={async () => { if (confirm('이 공지를 삭제하시겠습니까?')) { await manageFetch(`/schedule/announcements/${a.id}`, { method: 'DELETE' }); load() } }}
                      className="text-slate-300 hover:text-red-500 transition-colors" title="공지 삭제">
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                      </svg>
                    </button>
            ...
            <div key={p.id} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isLeader && (
                    <button onClick={async () => { if (confirm('이 투표를 삭제하시겠습니까?')) { await manageFetch(`/schedule/polls/${p.id}`, { method: 'DELETE' }); load() } }}
                      className="text-slate-300 hover:text-red-500 transition-colors" title="투표 삭제">
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                      </svg>
                    </button>

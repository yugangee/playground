...
import { Search, X, HelpCircle, Footprints, Shirt, CircleDot, Hand, Watch, Backpack, ShoppingCart, Heart, Package } from "lucide-react";

function MarketIcon({ name, size = 24, className = "" }: { name: string; size?: number; className?: string }) {
  const icons: Record<string, React.ReactNode> = {
    shoes: <Footprints size={size} className={className} />,
    shirt: <Shirt size={size} className={className} />,
    ball: <CircleDot size={size} className={className} />,
    glove: <Hand size={size} className={className} />,
    watch: <Watch size={size} className={className} />,
    bag: <Backpack size={size} className={className} />,
    cart: <ShoppingCart size={size} className={className} />,
    item: <Package size={size} className={className} />,
  };
...
interface GroupBuy {
  ...
  organizer: string; deadline: string; icon: string; description: string; joined: boolean;
}

const GROUP_BUYS: GroupBuy[] = [
  { id: 1, icon: "shoes", sport: "축구", category: "축구화", title: "나이키 팬텀 GX 엘리트 FG 팀 공동구매", targetPrice: 180000, unitPrice: 220000, minQty: 10, currentQty: 7, maxQty: 15, organizer: "박호현", deadline: "2026-03-15", description: "나이키 팬텀 GX 엘리트 FG 축구화 팀 공동구매입니다.", joined: false },
  { id: 2, icon: "shirt", sport: "축구", category: "유니폼", title: "서경 어벤져스 2026 시즌 유니폼 공동주문", targetPrice: 55000, unitPrice: 75000, minQty: 15, currentQty: 15, maxQty: 20, organizer: "이건율", deadline: "2026-03-08", description: "2026 시즌 서경 어벤져스 팀 유니폼 공동주문.", joined: true },
  { id: 3, icon: "ball", sport: "축구", category: "용품", title: "아디다스 챔피언스리그 공식 볼 10개 공구", targetPrice: 48000, unitPrice: 65000, minQty: 10, currentQty: 4, maxQty: 10, organizer: "김민수", deadline: "2026-03-20", description: "아디다스 UCL 공식 경기구 공동구매.", joined: false },
  { id: 4, icon: "ball", sport: "농구", category: "용품", title: "스팔딩 NBA 공식구 팀 공동구매", targetPrice: 58000, unitPrice: 78000, minQty: 6, currentQty: 6, maxQty: 8, organizer: "최상훈", deadline: "2026-03-10", description: "스팔딩 NBA 공식 경기구 6개 공동구매.", joined: false },
  { id: 5, icon: "glove", sport: "야구", category: "용품", title: "롤링스 야구 글러브 팀 공동구매", targetPrice: 140000, unitPrice: 185000, minQty: 8, currentQty: 3, maxQty: 12, organizer: "정다영", deadline: "2026-04-01", description: "롤링스 프로 시리즈 야구 글러브 공동구매.", joined: false },
];
...
interface Product {
  ...
  icon: string; sport: string; seller: string; region: string; views: number; likes: number;
}

const products: Product[] = [
  { id: 1, name: "나이키 머큐리얼 슈퍼플라이 10", category: "축구화", price: 280000, grade: "A", icon: "shoes", sport: "축구", seller: SELLERS[0], region: REGIONS[0], views: 142, likes: 12 },
  { id: 2, name: "아디다스 프레데터 엘리트", category: "축구화", price: 195000, grade: "C", icon: "shoes", sport: "축구", seller: SELLERS[1], region: REGIONS[1], views: 88, likes: 7 },
  { id: 3, name: "나이키 팬텀 GX 엘리트", category: "축구화", price: 230000, grade: "S", icon: "shoes", sport: "축구", seller: SELLERS[2], region: REGIONS[2], views: 201, likes: 25 },
  { id: 4, name: "퓨마 킹 플래티넘", category: "축구화", price: 142000, grade: "C", icon: "shoes", sport: "축구", seller: SELLERS[3], region: REGIONS[3], views: 55, likes: 3 },
  { id: 5, name: "손흥민 토트넘 홈 유니폼 23/24", category: "유니폼", price: 85000, grade: "S", icon: "shirt", sport: "축구", seller: SELLERS[4], region: REGIONS[0], views: 312, likes: 41 },
  { id: 6, name: "맨체스터 시티 홈 유니폼 24/25", category: "유니폼", price: 98000, grade: "S", icon: "shirt", sport: "축구", seller: SELLERS[5], region: REGIONS[5], views: 187, likes: 22 },
  { id: 7, name: "아디다스 챔피언스리그 공식 볼", category: "용품", price: 55000, grade: "A", icon: "ball", sport: "축구", seller: SELLERS[6], region: REGIONS[2], views: 67, likes: 5 },
  { id: 8, name: "나이키 에어 줌 BB NXT", category: "농구화", price: 210000, grade: "A", icon: "shoes", sport: "농구", seller: SELLERS[7], region: REGIONS[0], views: 133, likes: 18 },
  { id: 9, name: "조던 36 로우", category: "농구화", price: 245000, grade: "S", icon: "shoes", sport: "농구", seller: SELLERS[8], region: REGIONS[5], views: 298, likes: 37 },
  { id: 10, name: "NBA 레이커스 저지 23/24", category: "유니폼", price: 92000, grade: "S", icon: "shirt", sport: "농구", seller: SELLERS[9], region: REGIONS[3], views: 221, likes: 28 },
  { id: 11, name: "스팔딩 NBA 공식 경기구", category: "용품", price: 65000, grade: "A", icon: "ball", sport: "농구", seller: SELLERS[0], region: REGIONS[0], views: 88, likes: 9 },
  { id: 12, name: "롤링스 프로 야구 글러브", category: "용품", price: 165000, grade: "B", icon: "glove", sport: "야구", seller: SELLERS[1], region: REGIONS[5], views: 94, likes: 10 },
  { id: 13, name: "MLB 다저스 유니폼 오타니", category: "유니폼", price: 145000, grade: "S", icon: "shirt", sport: "야구", seller: SELLERS[2], region: REGIONS[4], views: 267, likes: 35 },
  { id: 14, name: "윌슨 프로 스태프 97 테니스 라켓", category: "용품", price: 320000, grade: "A", icon: "item", sport: "테니스", seller: SELLERS[3], region: REGIONS[3], views: 176, likes: 21 },
  { id: 15, name: "가민 포러너 955 GPS 워치", category: "스마트워치", price: 420000, grade: "A", icon: "watch", sport: "기타", seller: SELLERS[4], region: REGIONS[0], views: 199, likes: 24 },
  { id: 16, name: "나이키 스포츠 백팩 30L", category: "가방", price: 88000, grade: "S", icon: "bag", sport: "기타", seller: SELLERS[5], region: REGIONS[4], views: 108, likes: 12 },
];

type SortOption = "latest" | "price_asc" | "price_desc" | "views";
type MarketTab = "trade" | "group";
type PayMethod = "card" | "kakao" | "toss" | "naver";
interface PayItem { type: "trade" | "group"; id: number; name: string; price: number; icon: string; }
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-10">
              ...
              {filtered.map(product => (
                <div key={product.id} onClick={() => setSelected(product)} className="group cursor-pointer">
                  <div className="aspect-square bg-[#1E1E1E] border border-white/[0.08] rounded-2xl flex items-center justify-center text-5xl mb-4 group-hover:border-white/20 transition-colors relative">
                    <MarketIcon name={product.icon} size={48} className="text-white/60" />
                    <button onClick={e => toggleLike(product.id, e)} className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Heart size={16} className={liked.has(product.id) ? "fill-red-500 text-red-500" : "text-white/40"} />
                    </button>
                  </div>
                ...
                return (
                  <div key={gb.id} onClick={() => setSelectedGroup(gb)} className="bg-[#1E1E1E] border border-white/[0.08] rounded-2xl p-6 cursor-pointer hover:border-white/20 transition-colors">
                    <div className="flex items-start gap-5">
                      <div className="w-16 h-16 bg-black rounded-xl flex items-center justify-center shrink-0"><MarketIcon name={gb.icon} size={28} className="text-white/60" /></div>
                      ...
                      <button onClick={e => { e.stopPropagation(); gb.joined ? joinGroup(gb.id) : startPurchase({ type: "group", id: gb.id, name: gb.title, price: gb.targetPrice, icon: gb.icon }); }}
        ...
        {selected && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-6" onClick={() => setSelected(null)}>
            <div className="w-full max-w-md bg-[#1E1E1E] border border-white/[0.08] rounded-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
              <div className="aspect-video bg-black flex items-center justify-center"><MarketIcon name={selected.icon} size={72} className="text-white/60" /></div>
              <div className="p-6 space-y-5">
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={e => toggleLike(selected.id, e)} className="w-12 h-12 border border-white/[0.08] rounded-xl flex items-center justify-center hover:bg-white/5">
                    <Heart size={18} className={liked.has(selected.id) ? "fill-red-500 text-red-500" : "text-white/40"} />
                  </button>
                  ...
                  <button onClick={() => startPurchase({ type: "trade", id: selected.id, name: selected.name, price: selected.price, icon: selected.icon })}
        ...
        {selectedGroup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-6" onClick={() => setSelectedGroup(null)}>
            <div className="w-full max-w-md bg-[#1E1E1E] border border-white/[0.08] rounded-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
              <div className="aspect-video bg-black flex items-center justify-center"><MarketIcon name={selectedGroup.icon} size={72} className="text-white/60" /></div>
              <div className="p-6 space-y-5">
                <div className="space-y-2">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-6">
            <div className="w-full max-w-md bg-[#1E1E1E] border border-white/[0.08] rounded-2xl p-6 space-y-5">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => {
                  const newProd: Product = { id: Date.now(), name: form.name, category: form.category, price: Number(form.price), grade: form.grade, icon: "cart", sport: form.sport, seller: "나", region: form.region, views: 0, likes: 0 };
              ...
              {payDone ? (
                ...
                <>
                  <div className="flex items-center gap-4 pb-5 border-b border-white/[0.08]">
                    <MarketIcon name={payingItem.icon} size={40} className="text-white/60" />
                    <div>

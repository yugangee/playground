...
```bash
aws s3 sync out/ s3://playground-web --delete --exclude "uploads/*"
```
...
```json
"scripts": {
  "deploy": "npm run build && aws s3 sync out/ s3://playground-web --delete && aws cloudfront create-invalidation --distribution-id YOUR_DISTRIBUTION_ID --paths '/*'"
}
```
